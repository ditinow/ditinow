  ...
  $.ajax({
    ...
    success:function(response){
      ...
      if(responseObj.errorCode == "4"){
        ...
        $("#"+selector+"BroadcastTransactionResult").html("上链广播失败。 请检验提交的交易字节。");
      }else{
        $("#"+selector+"BroadcastTransactionResult").html("上链广播成功。 区块链标识:<br>"+responseObj.fullHash);
      };
  ...
  $.ajax({
    ...
    success:function(response){
      ...
      console.log(responseObj);
        $("#getAccountBalancesResult").append("<b>阿朵币ARDR:</b> " + responseObj.balances[1].balanceNQT/100000000 + " <br><b>火链币IGNIS:</b> " + responseObj.balances[2].balanceNQT/100000000);
  ...
  $.ajax({
    ...
    success:function(response){
      ...
      for(i=0; i<length; i++){
        ...
        if (newArrayObj[i].hasPhasingAssetControl == false){
          var phasing = "否";
        }else {
          var phasing = "是";
        };
...
$("#sendMessageForm").submit(function(event){
  ...
  $.ajax({
    ...
    success:function(response){
      ...
      $("#sendMessageTbl-fee").html(feeIgnis+"&nbsp;火链币Ignis");
  ...
  $.ajax({
    ...
    success:function(response){
      ...
      if (responseObj.fullHash == "failed"){
        var message = "<b style='color:red'>解密失败！谨防假冒。 </b><br>您提供的注册码本非由<code>YOUR_COMPANY_NAME_HERE</code>生成。";
        ...
        var note = '<br><br><p><b>注:</b> 演示版将不广播这段交易字节 - broadcasted: false. 商业版请下载<a href="download.html">DApp</a> 并阅读安装说明。<br>'
        var requestDTResult = "<b style='color:green'>申请通过！!</b><br> 产权通证编号 "+asset+" , 将被转让至您提供的: "+recipientRS+" 。本次事件的区块链标识:"+fullHash+".";
...
$("#processVoucherValidationForm").submit(function(event){
  ...
  if (formData[0].value != voucherObj.signature){
    ...
    $("#processVoucherResult").html("匹配失败! 系统收据无法找到匹配的提货单数字签字, 请复查。");
  }else{
    $("#processVoucherResult").html("匹配成功! 提货单数字签字与系统内的收据匹配，请进行下一步操作。");
    ...
    $("#processVoucherTbl-fee").html(fee);
...
function processVoucherFn(formData){
  ...
  processVoucherXhr.onreadystatechange = function() {
    if (processVoucherXhr.readyState == 4 && processVoucherXhr.status == 200) {
      ...
      var note = "<small><b>注: </b>转让并没有真正发生，为了保障演示功能的正常显示默认设定为不向区块链广播 name='broadcast' value='false', 商用版里这个限制被取消。<a href='download.html'>下载商用版</a></small>"
      $("#processVoucherResult").html("资产已经转让! 区块链标识: <br>"+ responseObj.signedTransaction.fullHash + "<br>" +note);
...
function decodeFileTokenFn(formData){
  ...
  decFileTokenXhr.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (valid==true){
        var message = "<b>验证成功! </b>上传文件与存证哈希匹配。上传文件的内容自存证起没有变动。";
      }else {
        var message = "<b>验证失败! </b>上传文件与存证哈希不匹配。上传文件的内容与存证哈希没有关联。";
      };
      ...
      $("#decFileTokenTbl-file").html("名字: "+fileName + ", 大小: " + fileSize+" , 最后修改时间:  " + lastModified[0] + " , " +lastModified[1]);
...
$("#issueAssetForm").submit(function(event){
  ...
  if((formData[5].value)=="midnight seven through card squeeze point lovely joke gone own ink ancient"){
    ...
    if (i>=1){
      for(k=0; k<i; k++){
        ...
        if (m > 0){
          ...
          setter = "未知钱包，附加信息由佚名机构提交。";
          duty = "未知, <a href='utilities.html'>联系对方</a>";
        };
        var index = k+1;
        displayTbl='<table class="table word-break table-bordered table-sm fs-6 table-bordered caption-top">'+
                   '<caption>附加信息 ' + index +'之'+i+' </caption>'+
                   '<tr><td class="td-1">提交机构</td><td class="td-2">' + setter +'</td></tr>'+
    ...
    var i= assetAccountObj.accountAssets.length;
      for (k=0; k<i; k++){
        ...
        if (m > 0){
          ...
          account = "未知地址。建议使用 <a href='evidence.html'>消息存证、求证</a> 验证钱包所有权，或者另外查询一件商品";
          duty = "未知, <a href='utilities.html'>联系对方</a>";
          $("#test-q7-yes").html("");
          $("#test-q7-no").html("&#x2716;");
        };
        displayTbl='<table class="table word-break table-bordered table-sm fs-6 table-bordered caption-top">'+
                   '<caption>当前拥有者 ' + index +'之'+i+' </caption>'+
                   '<tr><td class="td-1">机构名称</td><td class="td-2">' + account +'</td></tr>'+
  ...
  $.post(url,"requestType=getAssetTransfers&includeAssetInfo=true&asset="+asset,function(response){
    ...
    if (length>0) {
      for(var i=0; i<length; i++){
        ...
        if (m < 0){
          var sender = "未知钱包, 佚名让出机构。";
          var senderDuty = "未知, <a href='utilities.html'>联系对方</a>";
          senderLevel = 6;
        }else{
          ...
          if (n < 0){
            recipient = "未知钱包, 佚名接收机构。";
            recipientDuty = "未知, <a href='utilities.html'>联系对方</a>";
            recipientLevel = 6;
          }else{
        ...
        var index = length-i;
        displayTbl='<table class="table word-break table-bordered table-sm fs-6 table-bordered caption-top">'+
                   '<caption>交易 ' + index +'之'+length+' </caption>'+
                   '<tr><td class="td-1">&#x1F69A;接收机构</td><td class="td-2">' + recipient +'</td></tr>'+
                   '<tr><td class="td-1">&#x1F69A;接收钱包</td><td class="td-2">' + recipientRS +'</td></tr>'+
                   '<tr><td class="td-1">&#x1F69A;机构职责</td><td class="td-2">' + recipientDuty +'</td></tr>'+
                   '<tr><td class="td-1">&#x2708;让出机构</td><td class="td-2">' +sender +  '</td></tr>'+
                   '<tr><td class="td-1">&#x2708;让出钱包</td><td class="td-2">' + senderRS +'</td></tr>'+
                   '<tr><td class="td-1">&#x2708;机构职责</td><td class="td-2">' + senderDuty +'</td></tr>'+
                   '<tr><td class="td-1">转让数量</td><td class="td-2">' + qty + ' (quantityQNT: '+quantityQNT +', decimals: '+decimals+' )</td></tr>'+
                   '<tr><td class="td-1">转让时间</td><td class="td-2">' + localTime[0] +" "+ localTime[1]+ '</td></tr>'+
                   '<tr><td class="td-1">区块链标识</td><td class="td-2">' + fullHash + '</td></tr></table>';
    ...
    if (length>0) {
      for(var i=0; i<length; i++){
        ...
        displayTbl='<table class="table word-break table-bordered table-sm fs-6 table-bordered caption-top">'+
                   ...
                   '<tr><td class="td-1">&#x2708;让出钱包</td><td class="td-2">' + senderRS +'</td></tr>'+
                   '<tr><td class="td-1">&#x2708;机构职责</td><td class="td-2">' + senderDuty +'</td></tr>'+
                   '<tr><td class="td-1">交易数量</td><td class="td-2">' + qty + ' (quantityQNT: '+quantityQNT +', decimals: '+decimals+' )</td></tr>'+
                   '<tr><td class="td-1">交易时间</td><td class="td-2">' + localTime[0] +" "+ localTime[1]+ '</td></tr>'+
                   '<tr><td class="td-1">区块链标识</td><td class="td-2">' + fullHash + '</td></tr></table>';
        $("#assetIssuerTbl").before(displayTbl);
      };
    };
...
function genFileTokenFn(formData){
  ...
  genFileTokenXhr.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      var displayResult = "<p>钱包地址 "+accountRS +" 于当地时间 "+localTime[0]+" , "+localTime[1]+" 生成文件存证哈希。</p>"+
                          "<p>文件名字: " + fileName+" ,文件大小: "+fileSize+" , 最后修改时间: "+lastModified[0]+" , "+lastModified[1]+"。</p>"+
                          "<p>存证哈希: " + fileToken+" </p>";
...
$("#genTokenForm").submit(function(event){
  ...
  $.ajax({
    ...
    success:function(response){
      ...
      var localTime = timestampToLocalFn(timestamp,isNet);
      var result = "<span>零费用信息存证由账号 " +accountRS+" 于 "+ localTime[0] + " , " + localTime[1] + " 生成。<br>区块链标识： <br> "+token;
      $("#evidenceJSONBtn").removeClass("collapsed");
      $("#evidenceJSONPanel").addClass("show");
...
$("#decTokenForm").submit(function(event){
  ...
  $.ajax({
    ...
    success:function(response){
      ...
      var localTime = timestampToLocalFn(timestamp,isNet);
      if (valid==true){
        var displayResult = "<b>验证成功! </b> 输入信息与哈希匹配。哈希由账号 "+accountRS+" 于 "+ localTime[0] + " , " + localTime[1] + " 生成。";
      }else {
        var displayResult = "<b>验证失败! </b> 输入信息与哈希不匹配。哈希由账号 "+accountRS+" 于 "+ localTime[0] + " , " + localTime[1] + " 生成。";
      };
...
$("#passphraseEx2Form").submit(function(event){
  event.preventDefault();
  var formData = $(this).serializeArray();
  $("#genTokenDisplay2").removeClass("d-none");
  $.ajax({
    type:"POST",
    url: url,
    data:formData,
...
$("#splitSecretForm").submit(function(event){
  event.preventDefault();
  var formData = $(this).serializeArray();
  $("#splitSecretResultTitle").removeClass("d-none");
  $.ajax({
    type:"POST",
    url: url,
    data:formData,
    success:function(response){
      ...
      $("#splitSecretResult").append("复原钱包密钥请拷贝以上任意三段分片至以下表格, 例:");
    }
  });
});
...
$("#combineSecretForm").submit(function(event){
  event.preventDefault();
  var formData = $(this).serializeArray();
  $("#combineSecretResultTitle").removeClass("d-none");
  $.ajax({
    type:"POST",
    url: url,
    data:formData,
